...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ["in_repair", "available", "assigned"];
  if (!allowed.includes(asset.status)) {
    return NextResponse.json(
      { error: `Cannot retire asset in "${asset.status}" status.` },
      { status: 409 },
    );
  }
  ...
  // Close open maintenance logs for this asset
  const logs = await readTable("maintenance_logs");
  let logsChanged = false;
  for (const l of logs) {
    if (l.asset_id === id && !["closed", "resolved", "cancelled"].includes(l.status)) {
      l.status = "closed";
      l.updated_at = nowIso();
      l.completed_at = nowIso();
      l.notes = l.notes ? `${l.notes}; Asset retired: ${reason}` : `Asset retired: ${reason}`;
      logsChanged = true;
    }
  }
  ...
  if (logsChanged) await writeTable("maintenance_logs", logs);
  await writeAudit({
    actorUserId: user.id, actionType: "asset.retire",
